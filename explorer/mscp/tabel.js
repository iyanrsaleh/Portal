/*
 * Tampilan tabel dokumen "mscp" — dipanggil oleh ../index.js (renderDocForView)
 * lewat import('./mscp/tabel.js'), menerima `data` (hasil NxStorage('mscp'))
 ...
 * supaya bisa diubah bebas kapan saja KHUSUS untuk mscp (kolom, aksi, format
 * sel, filter, dll) tanpa menyentuh ipkd atau dokumen lain sama sekali.
 */
...
export async function render(container, data) {
    ...
    container.innerHTML = `
      <div style="padding: 24px; max-width:95%; margin: 0 auto;">
        ...
        <fluent-card style="padding: 16px;">
          <p style="margin: 0; font-size: 13px; color: var(--beranda-dv-text-muted);">Skema tabel (storage/mscp.json → production) tidak ditemukan.</p>
        </fluent-card>
      </div>
      ${backButtonHtml}
    `;
  ...
  container.innerHTML = `
    <div style="padding: 24px; max-width:95%; margin: 0 auto;">
      <h2 style="margin: 0 0 8px 0; font-size: 22px; font-weight: 600; color: var(--beranda-dv-text);">${data.appname || 'mscp'}</h2>
  ...
  await mountGeneratedTable(host, {
    ...
    onAction: (action, row) => {
      console.log('[explorer/mscp] row action:', action, row);
    },
    export: {
      enabled: true,
