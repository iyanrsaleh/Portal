/*
 * Tampilan form dokumen "ipkd" — dipanggil oleh ../index.js (renderDocForView)
 * lewat import('./ipkd/form.js'), menerima `data` (hasil NxStorage('ipkd'))
 ...
 * upload), supaya bisa diubah bebas kapan saja KHUSUS untuk ipkd (validasi,
    ...
    container.innerHTML = `
      <div style="padding: 24px; max-width: 560px; margin: 0 auto;">
        ...
        <fluent-card style="padding: 16px;">
          <p style="margin: 0; font-size: 13px; color: var(--beranda-dv-text-muted);">Skema form (storage/ipkd.json → production) tidak ditemukan.</p>
        </fluent-card>
      </div>
      ${backButtonHtml}
    `;
  ...
  const generatedForm = await mountGeneratedForm(formHost, data, {
    onSubmit: (values) => {
      ...
      NXUI.Storage().cloud().addBackground({
        tabel: data.production.tabel || 'ipkd',
        file: file_path,
        userid: 1,
    ...
    onReset: () => {
      console.log('[explorer/ipkd] form direset');
    },
  });
}
